...
export class FileService {
  private static WORKSPACE_ROOT = path.join(process.cwd(), 'workspaces');
  ...
  static async writeProjectFile(projectId: string, filePath: string, content: string) {
  ...
  static async readProjectFile(projectId: string, filePath: string): Promise<string> {
  ...
  static async listFiles(projectId: string): Promise<string[]> {
    ...
    try {
      ...
      return entries
        .filter(e => e.isFile())
        .map(e => path.relative(projectPath, path.join((e as any).path || (e as any).parentPath || '', e.name)));
    } catch (error) {
      return [];
    }
  }

  static async deleteFile(projectId: string, filePath: string) {
    const fullPath = path.join(this.getProjectPath(projectId), filePath);
  ...
  static async syncProjectToDisk(projectId: string, files: Record<string, string>) {
    const projectPath = this.getProjectPath(projectId);
