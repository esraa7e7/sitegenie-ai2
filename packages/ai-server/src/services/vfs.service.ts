...
export class VfsService {
   ...
   */
  static async autoSave(projectId: string, files: Array<{ path: string; content: string }>) {
    if (!redis) {
   ...
   */
  private static syncTimeouts = new Map<string, NodeJS.Timeout>();

  private static scheduleLazySync(projectId: string) {
    if (this.syncTimeouts.has(projectId)) return;
   ...
   */
  static async syncCacheToDatabase(projectId: string) {
    if (!redis) return { status: 'skipped_no_redis' };
    
    const key = `vfs:live:${projectId}`;
    const cachedFiles = await redis.get<any[]>(key);

    if (!cachedFiles || cachedFiles.length === 0) return { status: 'nothing_to_sync' };

    console.log(`[VFS] Syncing live cache to DB for project ${projectId}...`);

    const operations = cachedFiles.map(file => 
      prisma.file.upsert({
   ...
   */
  static async createSnapshot(projectId: string, reason: string = 'Manual Save') {
    // 1. Ensure latest state is in DB
    await this.syncCacheToDatabase(projectId);

    // 2. Fetch all files from DB
    const files = await prisma.file.findMany({ where: { projectId } });
    if (files.length === 0) throw new NotFoundError('No files found to snapshot.');
  ...
  static async restoreSnapshot(projectId: string, snapshotId: string) {
    ...
    const files = JSON.parse(decompress(snapshot.compressedData)) as any[];
   ...
   */
  private static async forceSyncToDatabase(projectId: string, files: any[]) {
   ...
   */
  static async syncProjectFiles(projectId: string, files: any[]) {
